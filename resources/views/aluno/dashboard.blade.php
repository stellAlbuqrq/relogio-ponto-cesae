...
@section('content')
    <div>
        <h1 class="ml-8 mt-4 mb-5 font-bold text-[#6A239B] text-4xl">Área do Aluno</h1>
        @if ($cronogramahoje->isEmpty())
            <p class="ml-8 mb-9 font-semibold text-[##232526] text-xl">
                Hoje não há aulas agendadas.
            </p>
        @else
            <div class="ml-8 mb-12">
                <p class="font-semibold text-[#232526] text-xl mb-5">Hoje as suas aulas são de:</p>
                <div class="flex flex-wrap justify-center gap-4">
                    @foreach ($cronogramahoje as $cronograma)
                        <div class="bg-[#f3ecf8] text-[#232526] text-xl font-bold px-5 py-3 rounded-2xl shadow">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-8">
        ...
        <div class="flex flex-col rounded-2xl bg-[#40155E] shadow-sm p-8 border border-slate-600">
            <div class="text-center">
                <p class="text-xl uppercase font-semibold text-[#EAEAEA]">EVOLUÇÃO DO CURSO</p>
            </div>
